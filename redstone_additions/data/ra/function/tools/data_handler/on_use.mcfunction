...
# Handle data handler click. As player (from advancement).

# Revoke advancement immediately
advancement revoke @s only ra:tools/data_handler_use

# Mark that player clicked this tick (for tag cleanup in tick.mcfunction)
tag @s add ra.dh_clicked

# If player is already holding (has tag), skip execution
execute if entity @s[tag=ra.dh_active] run return fail

# Mark player as actively using this tool
tag @s add ra.dh_active
...
execute unless data storage ra:temp dh_found run tellraw @s [{text:"[Data Handler] ",color:"gold"},{text:"No custom block found nearby.",color:"gray"}]
data remove storage ra:temp dh_found
