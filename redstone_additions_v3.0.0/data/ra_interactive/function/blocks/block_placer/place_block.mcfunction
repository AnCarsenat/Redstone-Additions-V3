...
execute positioned ^ ^ ^-1 if score #count ra.temp matches 1.. store result block ~ ~ ~ Items[0].count int 1 run scoreboard players get #count ra.temp
execute positioned ^ ^ ^-1 if score #count ra.temp matches ..0 run data remove block ~ ~ ~ Items[0]
