...
execute as @e[type=!armor_stand,type=!item_frame,type=!marker,type=!player,dx=0.5,dy=0.5,dz=0.5] at @s rotated as @e[type=marker,tag=ra.custom_block.pusher,distance=..3,sort=nearest,limit=1] run tp @s ^ ^ ^-0.5
execute as @a[distance=..0.9] at @s rotated as @e[type=marker,tag=ra.custom_block.pusher,distance=..3,sort=nearest,limit=1] run tp @s ^ ^ ^-1
